...
describe("authentication service", () => {
  ...
  describe("signup", () => {
    test("calls the backend url for a token", async () => {
      ...
      await signup(testFirstname, testLastname, testUsername, testEmail, testPassword);
      ...
      expect(options.method).toEqual("POST");
      expect(options.body).toEqual(
        JSON.stringify({ firstname: testFirstname, lastname: testLastname, username: testUsername, email: testEmail, password: testPassword})
      );
      expect(options.headers["Content-Type"]).toEqual("application/json");
    });
    ...
    test("returns nothing if the signup request was a success", async () => {
      ...
      const token = await signup(testFirstname, testLastname, testUsername, testEmail, testPassword);
